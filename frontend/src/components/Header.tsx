...
export default function Header({title, subtitle, children} : {title: string, subtitle?: string, children?: ReactNode}) {
        return (
            <header className="flex justify-center items-center flex-col w-full">
                <div className="lg:text-6xl text-5xl text-center font-extrabold py-2 bg-gradient-to-r from-primary to-primary-foreground inline-block text-transparent bg-clip-text">{title}</div>
                {subtitle ?
