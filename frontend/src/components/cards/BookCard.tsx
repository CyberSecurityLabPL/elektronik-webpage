...
export default function BookCard({subject, src, title, dist, url} : {subject: string, src: string, title: string, dist: string, url: string}){
    return (
        <Card className="flex w-fit aspect-[55/26] m-4 shadow-sm p-3">
            ...
            <div className="w-52 lg:w-[19rem] h-full">
                <CardHeader className="py-0 lg:pb-2">
                    <CardTitle className="font-semibold text-foreground text-lg lg:text-2xl">{subject}</CardTitle>
                </CardHeader>
                ...
                <CardFooter className="flex justify-center items-center w-full py-1 lg:py-0 my-0">
                    <Button className="max-lg:hidden text-xs lg:text-sm" asChild variant={"outline"}>
                        <Link href={url}>Kup Teraz</Link>
                    </Button>
                    <Button className="lg:hidden text-xs lg:text-sm p-[6px]" size={"sm"} asChild variant={"outline"}>
                        <Link href={url} className="w-fit h-fit">Kup Teraz</Link>
                    </Button>
                </CardFooter>
            </div>
        </Card>
